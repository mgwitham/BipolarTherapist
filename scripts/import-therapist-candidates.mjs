...
// Aggregator/nav strings that historically leaked into the city field when
// the discover script's regex misfired on Psychology Today (and similar)
// pages. Blocked at import time as defense-in-depth against bad data
// reaching Sanity even if the source CSV was generated by a different path.
...
function isContaminatedCity(value) {
  ...
  return CITY_CONTAMINATION_PREFIXES.some((prefix) => trimmed.startsWith(prefix));
}
...
function buildCandidateDocument(row, context, index) {
  ...
  if (isContaminatedCity(row.city)) {
    throw new Error(
      `City "${row.city}" looks like aggregator nav contamination (Psychology Today / TherapyDen / etc.). ` +
        `Fix the row's city to the clinician's real practice city before import. Problem row: ${JSON.stringify(row)}`,
    );
  }
