...
function renderProfile(t) {
  ...
  var contactBtns = "";
  if (t.phone)
    contactBtns += '<a href="tel:' + t.phone + '" class="btn-contact">📞 Call ' + t.phone + "</a>";
  if (t.email && t.email !== "contact@example.com")
    contactBtns +=
      '<a href="mailto:' +
      t.email +
      '" class="btn-contact" style="background:var(--teal-dark)">✉️ Email</a>';
  if (t.website)
    contactBtns +=
      '<a href="' +
      t.website +
  ...
  var specialtyTags = (t.specialties || [])
    .map(function (s) {
      return '<span class="spec-tag">' + s + "</span>";
    })
    .join("");

  var insTags = (t.insurance_accepted || [])
    .map(function (i) {
      return '<div class="ins-item">' + i + "</div>";
    })
    .join("");

  var langPills = (t.languages || ["English"])
    .map(function (l) {
  ...
  if (t.session_fee_min || t.session_fee_max) {
    feesHtml =
      '<div class="fee-range">$' +
      (t.session_fee_min || "") +
      (t.session_fee_max ? "–$" + t.session_fee_max : "") +
      "/session</div>";
  ...
  var html =
    ...
    (t.practice_name
      ? '<div class="title-text" style="color:var(--navy);font-weight:500">' +
        t.practice_name +
        "</div>"
      : "") +
    '<div class="location">📍 ' +
    ...
    "<div>" +
    '<div class="profile-section"><h2>About ' +
    t.name +
    '</h2><div class="bio-text">' +
    (t.bio || "No bio provided.") +
    "</div></div>" +
    (specialtyTags
      ? '<div class="profile-section"><h2>Specialties & Focus Areas</h2><div class="specialty-grid">' +
        specialtyTags +
        "</div></div>"
      : "") +
    (insTags
    ...
    '<div class="info-row"><span class="info-label">Status</span><span class="info-val green">' +
    (t.accepting_new_patients ? "Accepting Patients" : "Waitlist") +
    "</span></div>" +
    (t.years_experience
      ? '<div class="info-row"><span class="info-label">Experience</span><span class="info-val">' +
        t.years_experience +
        " years</span></div>"
      : "") +
    ...
    (!t.phone && (!t.email || t.email === "contact@example.com") && !t.website
      ? '<p style="font-size:.85rem;color:var(--muted)">Contact information not provided. Please visit their practice website.</p>'
      : "") +
    "</div></div></div>" +
