...
// Shapes a therapist document into the portal /me + PATCH response payload.
// One source of truth so /portal/me, /portal/therapist, and the claim flow
// never drift in what they expose. Pure (camelCase doc → snake_case wire).
export function shapePortalTherapist(therapist) {
  return {
    slug: therapist.slug,
    name: therapist.name,
...
export function buildTherapistDocument(application, existingId, helpers) {
  ...
  return {
    ...
    availabilityPosture: application.availabilityPosture || "",
    waitlistWeeks: helpers.parseNumber(application.waitlistWeeks),
    prescribingMode: application.prescribingMode || "",
    crisisPosture: application.crisisPosture || "",
    listingActive: true,
    status: "active",
  };
}
...
export function buildTherapistDocumentFromCandidate(candidate, existingId, helpers) {
  ...
  return {
    ...
    trainingAffiliations: helpers.splitList(candidate.trainingAffiliations),
    ...
    availabilityPosture: candidate.availabilityPosture || "",
    waitlistWeeks: helpers.parseNumber(candidate.waitlistWeeks),
    prescribingMode: candidate.prescribingMode || "",
    crisisPosture: candidate.crisisPosture || "",
    listingActive: true,
    status: "active",
  };
}
...
export function buildCandidateMergeFillFields(therapist, candidate, helpers) {
  ...
  takeList("trainingAffiliations", incoming.trainingAffiliations);
...
export function normalizePortableCandidate(doc, helpers) {
  return {
    ...
    availability_posture: doc.availabilityPosture || "",
    waitlist_weeks: typeof doc.waitlistWeeks === "number" ? doc.waitlistWeeks : null,
    prescribing_mode: doc.prescribingMode || "",
    crisis_posture: doc.crisisPosture || "",
    dedupe_status: doc.dedupeStatus || "unreviewed",
    dedupe_confidence: typeof doc.dedupeConfidence === "number" ? doc.dedupeConfidence : null,
    dedupe_reasons: Array.isArray(doc.dedupeReasons) ? doc.dedupeReasons : [],
    ...
    matched_candidate_id: doc.matchedCandidateId || "",
...
export function normalizePortableApplicationDocument(doc, helpers) {
  ...
  return normalizePortableApplication({
    ...
    accepts_in_person: doc.acceptsInPerson !== false,
    accepting_new_patients: doc.acceptingNewPatients !== false,
    years_experience: typeof doc.yearsExperience === "number" ? doc.yearsExperience : null,
    bipolar_years_experience:
      typeof doc.bipolarYearsExperience === "number" ? doc.bipolarYearsExperience : null,
