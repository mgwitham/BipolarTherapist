...
function buildPage(t) {
  ...
  // (1200×630). We point to a branded dynamic card generated by the
  // edge function at /api/og/therapists/<slug>, which lays out the
  // therapist's photo (or initials avatar fallback), name,
  // credentials, city, and an accepting-new-patients pill on the
  // brand palette — far better than handing crawlers a raw headshot
  // cropped to a weird aspect. The card is cached at the CDN edge,
  // so per-share latency is the same as a static image.
  const ogImage = `${ORIGIN}/api/og/therapists/${encodeURIComponent(t.slug)}.png`;
