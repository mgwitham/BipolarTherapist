...
export function buildPage(t) {
  ...
  // (1200×630). We point to the branded card that scripts/generate-og-cards.mjs
  // pre-renders at build time into dist/og/therapists/<slug>.png (see
  // shared/og-card.mjs for why this is a static file rather than a
  // serverless/edge function: X rejects the RGBA PNGs @vercel/og emits,
  // and the sharp flatten step needs a Linux build environment, not a
  // Vercel function). This SSR handler only serves slugs that haven't
  // landed in the static build yet, so the URL must match the same
  // ?v5 cache-busting token generate-seo-profile-pages.mjs uses —
  // bump both together if the card art changes.
  const ogImage = `${ORIGIN}/og/therapists/${encodeURIComponent(t.slug)}.png?v5`;
