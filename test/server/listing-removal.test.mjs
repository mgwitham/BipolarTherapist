...
test("listing-removal confirm: valid token flips listingActive to false and redirects to /remove?removed=ok", async () => {
  ...
  assert.equal(ctxWrap.httpResponse.headers.Location, "http://localhost:8787/remove?removed=ok");
...
test("listing-removal confirm: invalid token redirects to ?removed=expired", async () => {
  ...
  assert.equal(
    ...
    "http://localhost:8787/remove?removed=expired",
  );
});
...
test("listing-removal confirm: missing token redirects to ?removed=invalid", async () => {
  ...
  assert.equal(
    ctxWrap.httpResponse.headers.Location,
    "http://localhost:8787/remove?removed=invalid",
  );
});
