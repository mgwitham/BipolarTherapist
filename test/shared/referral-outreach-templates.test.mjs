...
test("tracking off: one direct link to the destination, no code, no /r/, no ?ref=", () => {
  ...
  // Tracking is off: a covered city links straight to its page, with no
  // attribution code, no /r/ redirect, and no ?ref= param.
...
test("outpatient_therapist gets refer-out copy and its own subjects", () => {
  ...
  assert.match(
    intro.body,
    /^https:\/\/www\.bipolartherapyhub\.com\/bipolar-therapists\/pasadena-ca\/$/m,
  );
...
test("prescriber gets medication-management copy, its own subjects, and the city link", () => {
  ...
  assert.ok(
    introLines.indexOf("https://www.bipolartherapyhub.com/bipolar-therapists/san-diego-ca/") < 6,
  );
  assert.doesNotMatch(intro.body, /\/r\//);
  assert.doesNotMatch(intro.body, /\?ref=/);
...
test("prescriber follow-up leads with the city list when a city is on file", () => {
  ...
  const lines = body.split("\n");
  const link = "https://www.bipolartherapyhub.com/bipolar-therapists/san-diego-ca/";
...
test("the resource email points at the print button instead of promising a file", () => {
  ...
  // Tracking off: direct link to the city page; copy names the city and says
  // that page prints.
  assert.ok(
    hasLinkLine(
      withCity.body,
  ...
  assert.doesNotMatch(withCity.body, /\/r\//);
