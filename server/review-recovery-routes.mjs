...
const RECOVERY_ROUTES = [
  { methods: ["GET"], path: "/portal/requests", handler: recoveryGetPortalRequests },
  { methods: ["POST"], path: "/portal/requests", handler: recoveryPostPortalRequests },
  {
    methods: ["POST"],
    path: "/portal/recovery-request",
    handler: recoveryPostPortalRecoveryRequest,
  },
  { methods: ["GET"], path: "/recovery-requests", handler: recoveryGetRecoveryRequests },
  {
    methods: ["POST"],
    pattern: /^\/recovery-requests\/([^/]+)\/approve$/,
    handler: recoveryPostRecoveryRequestsIdApprove,
  },
  {
    methods: ["POST"],
    pattern: /^\/recovery-requests\/([^/]+)\/reject$/,
    handler: recoveryPostRecoveryRequestsIdReject,
  },
  {
    methods: ["POST"],
    pattern: /^\/recovery-requests\/([^/]+)\/dismiss$/,
    handler: recoveryPostRecoveryRequestsIdDismiss,
  },
  {
    methods: ["POST"],
    pattern: /^\/recovery-requests\/([^/]+)\/resend-signin$/,
    handler: recoveryPostRecoveryRequestsIdResendSignin,
  },
  {
    methods: ["POST"],
    pattern: /^\/recovery-requests\/([^/]+)\/send-confirmation$/,
    handler: recoveryPostRecoveryRequestsIdSendConfirmation,
  },
  { methods: ["GET"], path: "/recovery-confirm", handler: recoveryGetRecoveryConfirm },
  { methods: ["POST"], path: "/recovery-confirm", handler: recoveryPostRecoveryConfirm },
  {
    methods: ["PATCH", "POST"],
    pattern: /^\/portal\/requests\/([^/]+)$/,
    handler: recoveryPatchPortalRequestsId,
  },
];

async function recoveryGetPortalRequests(context) {
  const { client, config, deps, origin, request, response } = context;
  const { normalizePortalRequest, sendJson } = context.deps;
...
async function recoveryPostPortalRequests(context) {
  const { config, origin, request, response } = context;
...
async function recoveryPostPortalRecoveryRequest(context) {
  const { client, config, deps, origin, request, response } = context;
  const contextRequestId = context.requestId;
  const { canAttemptPortalAuth, parseBody, recordPortalAuthAttempt, sendJson } = context.deps;
  if (
...
async function recoveryGetRecoveryRequests(context) {
  const { client, config, deps, origin, request, response } = context;
...
async function recoveryPostRecoveryRequestsIdApprove(context, match) {
  const { client, config, deps, origin, request, response } = context;
  const contextRequestId = context.requestId;
  const { parseBody, sendJson } = context.deps;
  const recoveryApproveMatch = match;
  if (!deps.isAuthorized(request, config)) {
    sendJson(response, 401, { error: "Unauthorized." }, origin, config);
    return true;
  }
...
async function recoveryPostRecoveryRequestsIdReject(context, match) {
  const { client, config, deps, origin, request, response } = context;
  const { parseBody, sendJson } = context.deps;
  const recoveryRejectMatch = match;
  if (!deps.isAuthorized(request, config)) {
    sendJson(response, 401, { error: "Unauthorized." }, origin, config);
    return true;
  }
...
async function recoveryPostRecoveryRequestsIdDismiss(context, match) {
  const { client, config, deps, origin, request, response } = context;
  const { parseBody, sendJson } = context.deps;
  const recoveryDismissMatch = match;
  if (!deps.isAuthorized(request, config)) {
    sendJson(response, 401, { error: "Unauthorized." }, origin, config);
    return true;
  }
...
async function recoveryPostRecoveryRequestsIdResendSignin(context, match) {
  const { client, config, deps, origin, request, response } = context;
  const { sendJson } = context.deps;
  const recoveryResendSigninMatch = match;
  if (!deps.isAuthorized(request, config)) {
    sendJson(response, 401, { error: "Unauthorized." }, origin, config);
    return true;
  }
...
async function recoveryPostRecoveryRequestsIdSendConfirmation(context, match) {
  const { client, config, deps, origin, request, response } = context;
  const { parseBody, sendJson } = context.deps;
  const recoverySendConfirmationMatch = match;
  if (!deps.isAuthorized(request, config)) {
    sendJson(response, 401, { error: "Unauthorized." }, origin, config);
    return true;
  }
  ...
  if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(channelEmail)) {
    sendJson(response, 400, { error: "Enter a valid confirmation channel email." }, origin, config);
    return true;
  }
...
async function recoveryGetRecoveryConfirm(context) {
  const { client, config, deps, origin, response, url } = context;
  const { sendJson } = context.deps;
...
async function recoveryPostRecoveryConfirm(context) {
  const { client, config, deps, origin, request, response } = context;
  const contextRequestId = context.requestId;
  const { parseBody, sendJson } = context.deps;
  const body = await parseBody(request);
...
async function recoveryPatchPortalRequestsId(context, match) {
  const { client, config, deps, origin, request, response } = context;
  const { normalizePortalRequest, parseBody, sendJson, updatePortalRequestFields } = context.deps;
  const portalRequestUpdateMatch = match;
  if (!deps.isAuthorized(request, config)) {
    sendJson(response, 401, { error: "Unauthorized." }, origin, config);
    return true;
  }
